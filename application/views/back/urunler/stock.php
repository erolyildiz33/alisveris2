						...
						<?php foreach ($secenekler as $secenek) { ?>
							...
							<div class="form-label-group in-border">
								<div class="form-group border" id="urunetiket<?=$secenek['id']?>">
									<div class=" p-3 mx-2">
										<select  multiple="multiple" id="option<?=$secenek['id']?>" data-option="<?=$secenek['id']?>"  name="my-select[]" class="searchable">
											<?=selectBoxAltSecenekler($secenek['id'])?>
										</select>
										<label for="urunetiket<?=$secenek['id']?>"><?=$secenek['name']?> Seçenekleri</label>
									</div>
								</div>
							</div>
						<?php } ?>
